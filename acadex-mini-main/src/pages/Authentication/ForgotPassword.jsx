...
const SignIn = () => {
    ...
    const handleReset = (e) => {
        ...
        try {
            ...
            if (step === 0)
                requestWithoutToken({ email: formData.email }, `password/email`).then(res => {
                    // setProfile(res.user);
                    // console.log('forgot pass', res);
                    setStep(1);
                    ToastAlert('Success!', "We sent you a code, please check your email.", "0");
                    setIsProcessing(false);
                    // setClientSecret(res.payment.client_secret)
                }).catch(err => {
                    console.log('forgot error 2', err);

                    setIsProcessing(false);
                });
            if (step === 1)
                requestWithoutToken({ code: formData.code }, `password/code/check`).then(res => {
                    // setProfile(res.user);
                    // console.log('forgot pass 1', res);
                    setIsProcessing(false);
                    setStep(2);
                    ToastAlert('Success!', "Code matched, reset your password.", "0");
                    // setClientSecret(res.payment.client_secret)
                }).catch(err => {
            ...
            if (step > 1)
                requestWithoutToken({ email: formData.email, code: formData.code, password: formData.password }, `password/reset`).then(res => {
                    // setProfile(res.user);
                    // console.log('forgot pass 2', res);
                    ToastAlert('Success!', "Password reset was successful!", "0");
                    ...
                    <div className="hidden w-full xl:block xl:w-1/2">
                        <div className="py-17.5 px-26 text-center">
                            <Link className="mb-5.5 inline-block" to="/">
                                <img className="hidden dark:block" src={LogoPng} alt="Logo" />
                                <img className="dark:hidden" src={LogoPng} alt="Logo" />
                            </Link>
                            ...
                            <form onSubmit={handleReset}>
                                ...
                                <div className="mb-5">
                                    ...
                                    <LoadingButton isLoading={isProcessing} type="submit" btnClasses="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90">
                                        {step === 0 ? 'SEND ME CODE' : null}
                                        {step === 1 ? 'VERIFY CODE' : null}
                                        {step > 1 ? 'RESET PASSWORD' : null}
                                    </LoadingButton>
                                </div>


                                <div className="p-4 mb-4 h-8 text-xl text-center text-sm text-danger rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
