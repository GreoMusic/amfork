...
print("----- LISA IS LOADING VERSION 0.4.3 (Natural Outputs - Deep Regex Debug Refined Logic) -----") # Increment version for clarity
...
def generate_lisa_response(
    ...
    if not user_text.strip() and not assignment_complete:
        if rubric_points_list:
            ...
            print(f"DEBUG: Initial Prompt - First rubric point for processing: {repr(first_rubric_point_lower)}") # Use repr from the start
            ...
            if detected_subject == "Creative Writing":
                ...
                # Prioritize specific patterns within academic subjects
                ...
                elif "introduction" in first_rubric_point_lower: # Most general "introduction" match
                    print(f"DEBUG: Initial Prompt - Found 'introduction' (general).")
                    example_intro = "strong introductory paragraph"
                else: # Fallback for academic subjects if no specific intro instruction
                    print(f"DEBUG: Initial Prompt - No specific intro instruction found for academic academic subject. Defaulting.")
                    example_intro = "strong introductory paragraph"
            else: # Fallback for other non-creative subjects
                print(f"DEBUG: Initial Prompt - Other subject type. Defaulting to general intro.")
