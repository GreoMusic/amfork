...
print("----- LISA IS LOADING VERSION 0.4.5 (Breadth and Depth Update) -----") # Increment version for clarity
...
# (Note: This NN is defined but not actively used for prediction in the current flow)
# ----------------------------------------------------\
...
def classify_subject(text, assignment_title):
    ...
    science_keywords_priority = ["climate", "environment", "biology", "chemistry", "physics", "ecology", "research", "experiment", "hypothesis", "lab", "data", "anatomy", "ecosystem", "molecule", "energy", "evolution"]
    history_keywords_priority = ["history", "social studies", "government", "civics", "geography", "economics", "politics", "culture", "event", "period", "war", "revolution", "society", "civilization", "constitution", "democracy"]
    
    for keyword in science_keywords_priority:
        if re.search(r'\b' + re.escape(keyword) + r'\b', text_lower):
            print(f"DEBUG: classify_subject - Matched Science keyword (priority): {keyword}")
            return "Science"
    
    for keyword in history_keywords_priority:
        if re.search(r'\b' + re.escape(keyword) + r'\b', text_lower):
            print(f"DEBUG: classify_subject - Matched History keyword (priority): {keyword}")
            return "History/Social Studies"

    # Priority 2: Assignment title keywords (less specific, like "essay" for ELA, "equation" for Math)
...
def generate_lisa_response(
    ...
    if not user_text.strip() and not assignment_complete:
        if rubric_points_list:
            ...
            if detected_subject == "Creative Writing":
                example_intro = "compelling opening hook or introduce your main character, setting the scene for your story."
                print(f"DEBUG: Initial Prompt - Creative Writing subject detected. Intro: {example_intro}")
            elif detected_subject == "Math":
                example_intro = "the first step to solving the problem, clearly outlining your approach to the equation or concept."
                print(f"DEBUG: Initial Prompt - Math subject detected. Intro: {example_intro}")
            elif detected_subject == "Art/Music":
                example_intro = "your initial concept, sketch, or the main theme of your composition. What is the core idea you want to convey?"
                print(f"DEBUG: Initial Prompt - Art/Music subject detected. Intro: {example_intro}")
            elif detected_subject == "Foreign Language":
                example_intro = "your first sentence in the target language, or how you'd begin to approach the conversation/topic in that language."
                ...
                if "clearly introduce the topic of" in first_rubric_point_lower:
                    ...
                    if match:
                        ...
                        example_intro = f"clear introduction to the topic of **{topic_phrase}**, setting the stage for your discussion."
                    else:
                    ...
                    example_intro = "concise thesis statement outlining your main arguments for the essay."
                    ...
                    example_intro = "strong introductory paragraph to set the tone for your work."
                ...
                example_intro = "clear introductory paragraph to get your ideas flowing."
        else: # Fallback if rubric_points_list is empty
            print("DEBUG: Initial Prompt - No rubric points found. Defaulting.")
            example_intro = "strong introductory paragraph to begin your work."
    ...
    if most_relevant_rubric_point_text and user_text.strip() and most_relevant_rubric_point_similarity > 0.4:
        ...
        # --- Specific Rephrasing Rules for Rubric Concepts (kept for precision) ---
        # These ensure the concept is framed well, e.g., "defining renewable energy comprehensively"
        if "discuss the reduction of greenhouse gas emissions and its effect on climate change" in most_relevant_rubric_point_text.lower():
            rephrased_rubric_concept = "reducing greenhouse gas emissions and its effect on climate change" 
        ...
        # Add more specific rephrasing rules here for other common rubric points if needed
        # Example for Math:
        elif "show your steps for solving the problem" in most_relevant_rubric_point_text.lower():
            rephrased_rubric_concept = "showing your steps for solving the problem"
        elif "analyze the use of color and light in the painting" in most_relevant_rubric_point_text.lower():
            rephrased_rubric_concept = "analyzing the use of color and light in the painting"


        # --- Diversify the follow-up question based on user progress/similarity ---
        # Expanded with more breadth and depth questions for a tutor-like feel
        ...
        if most_relevant_rubric_point_similarity > 0.75: # Higher threshold for "near completion" or good coverage
            ...
            lisa_response = (
                f"Keep up the great work! Your sentence on '{user_text_brief}' is a good start, but {chosen_question.lower()}" # Ensure the question starts lowercase after "but"
            )
            return lisa_response
...
def lisa_prompt():
    try:
        ...
        # Initialize predicted_class - this variable is currently NOT set by the SubjectClassifier NN
        # The SubjectClassifier NN is defined but not used for prediction here without training.
