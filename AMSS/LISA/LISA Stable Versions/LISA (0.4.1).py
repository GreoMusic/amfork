...
print("----- LISA IS LOADING VERSION 0.4.2 (Natural Outputs - Deep Regex Debug) -----") # Increment version for clarity
...
def generate_lisa_response(
    ...
    if not user_text.strip() and not assignment_complete:
        if rubric_points_list:
            ...
            print(f"DEBUG: Initial Prompt - First rubric point: {first_rubric_point_lower}") # Moved print here
            ...
            if detected_subject == "Creative Writing":
                ...
                if "introduction" in first_rubric_point_lower:
                    if "concise thesis statement outlining the main arguments" in first_rubric_point_lower:
                        ...
                        print(f"DEBUG: Initial Prompt - About to run regex on: {repr(first_rubric_point_lower)}") # ADDED repr()
                        match = re.search(r"clearly introduce the topic of (.+)", first_rubric_point_lower)
                        if match:
                            topic_phrase = match.group(1).strip()
                            # Remove trailing period if present, as it might be part of the extracted phrase
                            if topic_phrase.endswith('.'):
                                topic_phrase = topic_phrase[:-1]
                            print(f"DEBUG: Initial Prompt - Topic phrase extracted: {repr(topic_phrase)}") # ADDED repr()
                            example_intro = f"clear introduction to the topic of **{topic_phrase}**"
                        else: # Fallback if for some reason the simpler regex still fails
                            print("DEBUG: Initial Prompt - Regex match failed for topic extraction.") # ADDED DEBUG
            ...
            example_intro = "strong introductory paragraph" # Default generic intro if no rubric points
            first_rubric_point_lower = "N/A - No rubric points found." # For debug print if needed
    ...
    if most_relevant_rubric_point_text and user_text.strip() and most_relevant_rubric_point_similarity > 0.4:
        ...
        print(f"DEBUG: User text before brief processing: '{user_text_brief}'") # ADDED DEBUG
        
        # Reordered and refined acknowledgements for keywords in user's text
        # Prioritize most specific matches first, then individual specifics, then general fallback
        if ("greenhouse gas" in user_text.lower() or "ghg" in user_text.lower()) and \
           ("carbon capturing" in user_text.lower() or "carbon capture" in user_text.lower()):
            user_text_brief = "your points on greenhouse gases and carbon capturing"
            print("DEBUG: user_text_brief set by combined GHG + Carbon Capturing.") # ADDED DEBUG
        elif "greenhouse gas" in user_text.lower() or "ghg" in user_text.lower():
            user_text_brief = "your point about greenhouse gases" 
            print("DEBUG: user_text_brief set by GHG.") # ADDED DEBUG
        elif "carbon capturing" in user_text.lower() or "carbon capture" in user_text.lower():
            user_text_brief = "your idea about carbon capturing technology"
            print("DEBUG: user_text_brief set by Carbon Capturing.") # ADDED DEBUG
        elif "climate change" in user_text.lower():
            user_text_brief = "your idea about climate change"
            print("DEBUG: user_text_brief set by Climate Change.") # ADDED DEBUG
        else: # Fallback to generic truncation if no specific keyword matches
            if len(user_text_brief) > 70: 
                user_text_brief = user_text_brief[:67] + "..."
                print("DEBUG: user_text_brief set by truncation.") # ADDED DEBUG
            # Else, keep the original user_text_brief (which is the full user_text.strip())
            print("DEBUG: user_text_brief kept as original (no specific match/truncation needed).") # ADDED DEBUG

        print(f"DEBUG: Final user_text_brief: '{user_text_brief}'") # ADDED DEBUG
    ...
    # These are placeholders. Replace them with your actual existing default logic for LISA.
    if inactivity_timer_fired:
        ...
        return "Keep writing! I'm here to help you develop your ideas." # Generic default if nothing else matches
