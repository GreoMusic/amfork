...
LISA_VERSION = "0.4.11 (Initial Guidance with Examples)" # Increment version for clarity
...
PRIORITY_SCIENCE_KEYWORDS = ["climate", "environment", "biology", "chemistry", "physics", "ecology", "research", "experiment", "hypothesis", "lab", "data", "anatomy", "physiology", "ecosystem", "cell", "molecule", "atom", "reaction", "force", "energy", "matter", "observation", "conclusion", "theory", "evolution", "genetics", "earth science", "quantum", "mechanics"]
...
def get_initial_guidance(detected_subject, rubric_points_list, assignment_title):
    ...
    example_intro_templates = {
        ...
        "Reading/ELA": [
            "An example could be: 'In this analysis of {subject_core}, we will examine how the author uses literary devices to convey...'",
    ...
    if detected_subject in ["Science", "History/Social Studies", "Reading/ELA", "General"]:
        ...
        intro_patterns = [
            (r"(?:clearly\s+)?introduce(?: the topic of)?\s*(.+?)(?:\.|\s*and\s*its\s*impact|\s*that\s*is\s*relevant\s+to\s+the\s+assignment|$|\s*and\s*briefly\s+outline\s+your\s+main\s+points|or\s+provide\s+an\s+overview)",
             lambda m: f"How about starting with a clear introduction to the topic of **{m.group(1).strip().strip('.')}**, setting the stage for your discussion?"),
            ...
            (r"(?:provide|begin with|start with)\s+(?:a\s+)?(?:clear\s+)?(?:general\s+)?(?:brief\s+)?(?:introduction|overview|context)(?: to| of)?(?:\s*(.+?))?(?:\.|$|\s*as\s+it\s+relates\s+to\s+the\s+assignment)",
...
def generate_lisa_response(
    ...
    if not user_text.strip() and not assignment_complete:
        # MODIFIED: Pass the full rubric_points_list AND assignment_title to get_initial_guidance
